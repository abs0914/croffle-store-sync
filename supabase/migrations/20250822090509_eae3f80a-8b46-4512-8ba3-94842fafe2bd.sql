-- Re-run inventory sync with correct logic and better error handling
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- If product has a recipe, process ingredients
            IF transaction_item.recipe_id IS NOT NULL THEN
                ...
                LOOP
                    ...
                    RAISE NOTICE 'Ingredient: % - Need: % %', ingredient_record.ingredient_name, total_qty_needed, ingredient_record.unit;
                    ...
                    IF FOUND THEN
                        ...
                        IF inventory_record.stock_quantity >= total_qty_needed THEN
                            ...
                            INSERT INTO inventory_transactions (
                                ...
                                'Manual inventory sync for transaction: ' || transaction_record.receipt_number || ' - ' || ingredient_record.ingredient_name,
                                transaction_record.id::TEXT,
                                NOW()
                            );
                            
                            RAISE NOTICE 'SUCCESS: Deducted % % of % from inventory (new stock: %)', 
                        ...
                        END IF;
                    ELSE
                        RAISE NOTICE 'INVENTORY NOT FOUND: % (%)', ingredient_record.ingredient_name, ingredient_record.unit;
                        -- Show available inventory items for debugging
                        FOR inventory_record IN
                            SELECT item, unit FROM inventory_stock 
                            WHERE store_id = transaction_record.store_id 
                              AND is_active = true
                              AND UPPER(item) LIKE '%WHIPPED%'
                        LOOP
                            RAISE NOTICE 'Available whipped cream inventory: % (%)', inventory_record.item, inventory_record.unit;
                        END LOOP;
                    END IF;
                END LOOP;
            ELSE
    ...
    RAISE NOTICE 'Inventory sync process completed';
END $$;