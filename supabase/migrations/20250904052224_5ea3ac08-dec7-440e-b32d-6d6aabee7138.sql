...
CREATE TABLE public.journal_entries (
  ...
  reference_type TEXT, -- 'transaction', 'expense', 'adjustment', 'closing'
  reference_id UUID,
...
CREATE TABLE public.fiscal_periods (
  ...
  period_year INTEGER NOT NULL,
  period_month INTEGER NOT NULL CHECK (period_month BETWEEN 1 AND 12),
  period_name VARCHAR(50) NOT NULL, -- 'January 2024', 'Q1 2024', etc.
...
CREATE TABLE public.financial_adjustments (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  store_id UUID REFERENCES public.stores(id),
  adjustment_type TEXT NOT NULL, -- 'accrual', 'prepayment', 'depreciation', 'correction'
...
-- Equity
('3000', 'Owner\'s Equity', 'equity', 'capital', true, 'Owner\'s investment in the business'),
('3100', 'Retained Earnings', 'equity', 'retained_earnings', true, 'Accumulated profits/losses'),
('3200', 'Current Year Earnings', 'equity', 'current_earnings', true, 'Current period profit/loss'),
...
-- RLS Policies - Admin and accounting users only
...
CREATE POLICY "Admins can view account balances" ON public.account_balances
...
CREATE INDEX idx_account_balances_period ON public.account_balances(period_year, period_month);
...
CREATE INDEX idx_fiscal_periods_store ON public.fiscal_periods(store_id, period_year, period_month);
...
CREATE TRIGGER update_account_balances_updated_at BEFORE UPDATE ON public.account_balances FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_fiscal_periods_updated_at BEFORE UPDATE ON public.fiscal_periods FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_financial_adjustments_updated_at BEFORE UPDATE ON public.financial_adjustments FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();