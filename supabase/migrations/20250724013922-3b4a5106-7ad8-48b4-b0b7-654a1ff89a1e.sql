...
-- Update minimum_threshold if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'inventory_stock' AND column_name = 'minimum_threshold') THEN
        EXECUTE 'ALTER TABLE inventory_stock ALTER COLUMN minimum_threshold TYPE NUMERIC';
    END IF;
END $$;

-- Update maximum_capacity if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'inventory_stock' AND column_name = 'maximum_capacity') THEN
        EXECUTE 'ALTER TABLE inventory_stock ALTER COLUMN maximum_capacity TYPE NUMERIC';
    END IF;
END $$;
...
BEGIN
  -- Delete existing alerts for this item (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'store_inventory_alerts') THEN
...
RETURNS TRIGGER AS $$
BEGIN
    -- Update serving_ready_quantity if column exists
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'inventory_stock' AND column_name = 'serving_ready_quantity') THEN
