...
-- Step 3: Clean up inventory_stock table - remove unused columns
...
-- Step 4: Restructure inventory_stock for direct unit system only
ALTER TABLE inventory_stock 
...
ADD COLUMN IF NOT EXISTS unit_type TEXT DEFAULT 'serving';

-- Add constraint for unit_type
ALTER TABLE inventory_stock 
DROP CONSTRAINT IF EXISTS inventory_stock_unit_type_check;
ALTER TABLE inventory_stock 
ADD CONSTRAINT inventory_stock_unit_type_check 
CHECK (unit_type IN ('pieces', 'serving', 'portion', 'scoop', 'grams', 'ml'));

-- Step 5: Update all existing inventory to use direct serving units
...
-- Step 6: Clean up recipe tables - remove complex conversion fields
...
-- Step 7: Clean up recipe template ingredients
...
-- Step 8: Remove old conversion-related functions
...
-- Step 9: Remove old triggers related to conversions
...
-- Step 10: Create simplified inventory update function
...
-- Step 11: Apply simplified trigger
...
-- Step 12: Clean up any remaining conversion-related data
...
-- Step 13: Update product_ingredients to use direct serving units (if exists)
...
-- Step 14: Clean up old migration tracking
DROP TABLE IF EXISTS migration_tracking;

-- Step 15: Create validation function for clean slate
...
-- Step 16: Set default minimum thresholds for items without them
UPDATE inventory_stock 
SET minimum_threshold = 10 
WHERE minimum_threshold IS NULL OR minimum_threshold = 0;

-- Step 17: Final data consistency check and cleanup
UPDATE inventory_stock 
SET 
    serving_ready_quantity = stock_quantity,
