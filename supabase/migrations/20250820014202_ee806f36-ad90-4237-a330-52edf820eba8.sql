-- Croffle Recipe Correction with Correct Enum Values
-- This migration fixes all croffle recipes with correct ingredient lists using valid inventory_unit enum values
...
-- Create helper function to add recipe ingredients with correct enum values
...
BEGIN
    ...
    FOR i IN 1..array_length(ingredients, 1) LOOP
        ...
        -- Set quantities and units based on ingredient type using valid enum values
        CASE 
            ...
            WHEN ingredient_name LIKE '%Toppings%' OR ingredient_name LIKE '%Flakes%' OR ingredient_name LIKE '%Crumbs%' THEN
                ingredient_qty := 10; ingredient_unit := 'g'::inventory_unit;
            WHEN ingredient_name LIKE '%1pc%' THEN
                ingredient_qty := 1; ingredient_unit := 'pieces'::inventory_unit;
            WHEN ingredient_name LIKE '%Crushed%' THEN
                ingredient_qty := 5; ingredient_unit := 'g'::inventory_unit;
