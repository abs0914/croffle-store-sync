-- COMPREHENSIVE COFFEE RECIPE INGREDIENT CORRECTIONS
...
INSERT INTO recipe_template_ingredients (recipe_template_id, ingredient_name, quantity, unit, cost_per_unit, created_at, updated_at)
SELECT 
    rt.id,
    'Bending Straw',
    1,
...
-- Update existing Choco Flakes to portions
UPDATE recipe_template_ingredients 
SET 
    quantity = CASE 
        WHEN recipe_template_id IN (
...
-- Update Oreo Cookie unit from piece to pc
UPDATE recipe_template_ingredients 
SET 
    unit = 'pc',
    updated_at = NOW()
...
-- Update Crushed Oreo to 3 portions
UPDATE recipe_template_ingredients 
SET 
    quantity = 3,
    unit = 'portions',
...
-- Update Choco Flakes in deployed recipes
UPDATE recipe_ingredients ri
SET 
    quantity = CASE 
...
-- Update Oreo ingredients in deployed recipes
UPDATE recipe_ingredients ri
SET 
    quantity = CASE 
    ...
    END,
    unit = CASE 
        WHEN ri.ingredient_name = 'Oreo Cookie' THEN 'pc'
        WHEN ri.ingredient_name = 'Crushed Oreo' THEN 'portions'
        ELSE ri.unit
    END,
    updated_at = NOW()
FROM recipes r
...
-- Verification: Check sample of updated ingredients
SELECT 
