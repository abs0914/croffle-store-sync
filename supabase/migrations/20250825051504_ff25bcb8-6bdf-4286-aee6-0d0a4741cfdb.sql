-- Fix the type casting issue in the function
    ...
    LOOP
      ...
      LOOP
        ...
        IF NOT EXISTS (
          ...
          SELECT ist.id, ist.cost, ist.unit 
          ...
          -- Only add if inventory item exists
          IF inventory_item_record.id IS NOT NULL THEN
            -- Determine the unit to use
            BEGIN
              ingredient_unit := inventory_item_record.unit::inventory_unit;
            ...
            INSERT INTO recipe_ingredients (
              ...
              END,
              ingredient_unit,
              COALESCE(inventory_item_record.cost, 1.0),
              inventory_item_record.id,
              NOW(),
              NOW()
            );
...
-- Now execute the function to complete all recipes
