...
-- 3. Update inventory stock costs to match specifications
...
SET 
  ...
  END,
  unit = CASE 
    WHEN item = 'Whipped Cream' THEN 'serving'
    WHEN item = 'Caramel Sauce' THEN 'portion'
    WHEN item = 'Colored Sprinkles' THEN 'portion'
    WHEN item = 'Chocolate Sauce' THEN 'portion'
    WHEN item = 'Marshmallow' THEN 'portion'
    WHEN item = 'Chopstick' THEN 'pair'
    ELSE unit
  END,
  updated_at = NOW()
WHERE item IN (
...
-- 4. Update all deployed recipes with corrected ingredients and costs
-- First, update existing recipe ingredients for both products
