-- Add missing add-on products to all stores
...
store_categories AS (
  SELECT 
    ...
    c.id as category_id
  FROM stores s
  LEFT JOIN categories c ON s.id = c.store_id AND c.name = 'Add-ons' AND c.is_active = true
...
FROM stores s
JOIN categories c ON s.id = c.store_id AND c.name = 'Add-ons'
