...
-- 3. Clear store metrics for today for this store (correct column name is metric_date)
...
-- 8. Clear any remaining backup tables for this store for today (if they exist)
DELETE FROM transactions_backup_reset 
WHERE store_id = 'd7c47e6b-f20a-4543-a6bd-000398f72df5'
  AND DATE(created_at) = CURRENT_DATE;

DELETE FROM transaction_items_backup_reset 
WHERE transaction_id IN (
  SELECT id FROM transactions_backup_reset 
  WHERE store_id = 'd7c47e6b-f20a-4543-a6bd-000398f72df5'
    AND DATE(created_at) = CURRENT_DATE
);