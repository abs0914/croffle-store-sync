-- Phase 1: Create missing beverage recipes with proper ingredient mapping (Fixed enum casting)
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN ist.item ILIKE '%ice%' THEN 'grams'::inventory_unit
    END,
    NOW()
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      ...
      END,
      CASE 
        WHEN ist.item ILIKE '%matcha%' OR ist.item ILIKE '%frappe%' THEN 'grams'::inventory_unit
        WHEN ist.item ILIKE '%milk%' THEN 'ml'::inventory_unit
        WHEN ist.item ILIKE '%ice%' THEN 'grams'::inventory_unit
      END,
      NOW()
  ...
  LOOP
    ...
    SELECT 
      ...
      CASE 
        WHEN ist.item ILIKE '%strawberry%' OR ist.item ILIKE '%milk%' THEN 'ml'::inventory_unit
        WHEN ist.item ILIKE '%frappe%' OR ist.item ILIKE '%ice%' OR ist.item ILIKE '%chocolate%' THEN 'grams'::inventory_unit
      END,
      NOW()
    FROM inventory_stock ist
    WHERE ist.store_id = store_id_var
  ...
  LOOP
    ...
    SELECT 
      ...
      CASE 
        WHEN ist.item ILIKE '%strawberry%' OR ist.item ILIKE '%milk%' THEN 'ml'::inventory_unit
        WHEN ist.item ILIKE '%frappe%' OR ist.item ILIKE '%oreo%' OR ist.item ILIKE '%ice%' THEN 'grams'::inventory_unit
      END,
      NOW()
    FROM inventory_stock ist
    WHERE ist.store_id = store_id_var
  ...
  LOOP
    ...
    SELECT 
      ...
      END,
      CASE 
        WHEN ist.item ILIKE '%tea%' AND ist.item ILIKE '%powder%' THEN 'grams'::inventory_unit
  ...
  LOOP
    ...
    SELECT 
      ...
      END,
      CASE 
        WHEN ist.item ILIKE '%lemonade%' AND ist.item ILIKE '%powder%' THEN 'grams'::inventory_unit
        WHEN ist.item ILIKE '%water%' THEN 'ml'::inventory_unit
        WHEN ist.item ILIKE '%ice%' THEN 'grams'::inventory_unit
      END,
      NOW()
