...
-- The error "permission denied for table users" indicates functions are accessing auth.users directly

-- First, let's audit which functions might be accessing auth.users table incorrectly
-- and fix them to use app_users instead

-- Update any RPC functions that access auth.users to use app_users instead
-- This is likely in functions called during inventory sync
...
-- Update the is_admin_or_owner function to use app_users only
...
-- Update is_store_accessible function to use app_users only
...
-- Update user_has_store_access function to use app_users only
