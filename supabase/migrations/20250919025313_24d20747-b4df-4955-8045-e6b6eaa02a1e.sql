-- Create improved deployment function that handles duplicates
CREATE OR REPLACE FUNCTION deploy_recipe_templates_to_all_stores_v3()
RETURNS TABLE(
  ...
  LOOP
    ...
    LOOP
      ...
      IF existing_recipe_id IS NOT NULL THEN
        ...
        -- Ensure product catalog entry exists (with conflict resolution)
        BEGIN
          ...
          WHERE NOT EXISTS (
            ...
            WHERE pc.store_id = store_record.id 
              AND (pc.recipe_id = existing_recipe_id OR pc.product_name = template_record.name)
          );
      ...
      -- Create product catalog entry with template suggested price (with conflict resolution)
      BEGIN
        INSERT INTO product_catalog (
          store_id,
          product_name,
