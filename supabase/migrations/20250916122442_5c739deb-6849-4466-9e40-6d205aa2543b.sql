-- Fix backfill function to handle JSON array correctly
...
BEGIN
  ...
  LOOP
    ...
    IF jsonb_typeof(v_transaction.items) = 'array' THEN
      ...
      END LOOP;
    ELSE
      -- If items is not an array, log a warning and skip
      RAISE WARNING 'Transaction % has items field that is not an array (type: %)', v_transaction.id, jsonb_typeof(v_transaction.items);
    END IF;
  END LOOP;
