...
-- Fix critical security gaps identified in the linter
...
-- Add inventory stock policies for real-time monitoring
CREATE POLICY "inventory_stock_realtime_policy" ON public.inventory_stock
    FOR SELECT USING (
        -- Allow real-time monitoring for accessible stores
        is_store_accessible(store_id)
    );
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_store_created 
    ON public.transactions(store_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_inventory_stock_store_item 
    ON public.inventory_stock(store_id, item) WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_inventory_movements_stock_created 
    ON public.inventory_movements(inventory_stock_id, created_at DESC);
