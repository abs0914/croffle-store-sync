-- Final recipe system fix (simplified approach without trigger manipulation)
...
INSERT INTO inventory_stock (store_id, item, unit, stock_quantity, cost, minimum_threshold, maximum_capacity, is_active, item_category)
SELECT DISTINCT
  ur.store_id,
  uri.ingredient_name,
  CASE 
  ...
  true,
  CASE 
    WHEN uri.ingredient_name ILIKE '%coffee%' OR uri.ingredient_name ILIKE '%bean%' THEN 'Coffee & Beans'
    WHEN uri.ingredient_name ILIKE '%milk%' OR uri.ingredient_name ILIKE '%cream%' THEN 'Dairy'
    WHEN uri.ingredient_name ILIKE '%syrup%' OR uri.ingredient_name ILIKE '%sauce%' THEN 'Syrups & Sauces'
    WHEN uri.ingredient_name ILIKE '%sugar%' THEN 'Sweeteners'
    ELSE 'General'
