-- Recipe System Complete Reset Migration
...
-- Step 3: Clear recipe deployment tracking
DELETE FROM recipe_deployments;

-- Step 4: Add missing columns to recipe_templates for better management
...
-- Step 5: Add missing columns to recipes for deployment tracking
ALTER TABLE recipes 
ADD COLUMN IF NOT EXISTS recipe_type TEXT DEFAULT 'regular' CHECK (recipe_type IN ('regular', 'addon', 'combo', 'beverage')),
ADD COLUMN IF NOT EXISTS sku TEXT,
ADD COLUMN IF NOT EXISTS preparation_time INTEGER DEFAULT 0,
...
-- Step 6: Improve recipe_template_ingredients table
...
-- Step 7: Improve recipe_ingredients table  
ALTER TABLE recipe_ingredients 
ADD COLUMN IF NOT EXISTS commissary_item_id UUID REFERENCES commissary_inventory(id),
ADD COLUMN IF NOT EXISTS notes TEXT;

-- Step 8: Create recipe deployment errors table if not exists
...
-- Step 10: Create function to calculate recipe cost from template
...
-- Step 11: Create function to generate unique recipe SKU
...
-- Step 12: Update recipe cost calculation trigger
...
-- Step 13: Create validation function for recipe deployment
...
-- Step 14: Create comprehensive recipe deployment log
...
UPDATE recipe_templates SET 
  total_cost = calculate_template_cost(id),
  updated_at = now()
WHERE total_cost = 0 OR total_cost IS NULL;

-- Step 16: Create index for better performance
