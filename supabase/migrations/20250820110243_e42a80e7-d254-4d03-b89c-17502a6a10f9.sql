-- Comprehensive Inventory Sync Fix
-- Phase 1: Mass Duplicate Product Cleanup and Recipe Template Association

-- First, let's create a temporary table to track what we're cleaning up
...
-- Function to find and fix recipe template associations
...
-- Function to clean up duplicate products
...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 2..array_length(duplicate_group.product_ids, 1) LOOP
      ...
      -- Mark duplicate as inactive instead of deleting to preserve referential integrity
      UPDATE products
      SET is_active = false, name = name || '_DUPLICATE_' || extract(epoch from now())::text
...
SELECT * FROM cleanup_duplicate_products();
...
-- Update the product uniqueness trigger to be more comprehensive
...
-- Create comprehensive inventory validation view
...
SELECT 
  ...
  CASE 
    ...
    WHEN r.id IS NULL THEN 'no_recipe'
    WHEN rt.id IS NULL THEN 'no_template'
    WHEN rt.is_active = false THEN 'inactive_template'
    ELSE 'valid'
  END as status,
  validate_product_has_recipe_template(p.id) as can_deduct_inventory
FROM products p
