-- Fix column name mismatch in auto_deduct_inventory_on_transaction function
-- Change 'current_stock' to 'stock_quantity' to match the actual inventory_stock table schema
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            IF inventory_item.id IS NOT NULL THEN
                -- Check current stock - FIXED: Use stock_quantity instead of current_stock
                current_stock := inventory_item.stock_quantity;
                
                IF current_stock >= required_quantity THEN
                    -- Deduct from inventory - FIXED: Use stock_quantity instead of current_stock
                    UPDATE inventory_stock 
                    SET 
                        stock_quantity = stock_quantity - required_quantity,
                        updated_at = NOW()
    ...
    -- Log completion with correct sync_status value
