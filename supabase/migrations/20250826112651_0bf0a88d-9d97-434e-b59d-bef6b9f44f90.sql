-- Fix the deployment function to handle unit type casting
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF existing_recipe_id IS NOT NULL THEN
        ...
        IF existing_ingredient_count < template_ingredient_count THEN
          ...
          SELECT 
            ...
            rti.ingredient_name,
            rti.quantity,
            rti.unit::inventory_unit, -- Cast to proper enum type
            rti.cost_per_unit,
            NOW(),
            NOW()
          FROM recipe_template_ingredients rti
        ...
        SELECT 
          ...
          COALESCE(template_record.suggested_price, 100.00), -- Use template suggested price
          existing_recipe_id,
      ...
      -- Copy template ingredients to recipe ingredients with proper type casting
      INSERT INTO recipe_ingredients (
        recipe_id,
        ingredient_name,
        quantity,
