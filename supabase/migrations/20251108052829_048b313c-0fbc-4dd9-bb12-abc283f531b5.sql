-- Create a function to delete test transactions for a store
...
BEGIN
  ...
  WHERE im.reference_type = 'transaction'
    AND NOT EXISTS (
      SELECT 1 FROM transactions t WHERE t.id::text = im.reference_id
    );

  RETURN QUERY SELECT v_deleted_count, v_deleted_receipts;
END;
$$;