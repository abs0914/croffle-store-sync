...
-- Create simplified backup tables
CREATE TABLE IF NOT EXISTS deleted_sugbo_transactions_backup AS
SELECT 
  *,
...
CREATE TABLE IF NOT EXISTS deleted_sugbo_purchase_orders_backup AS  
SELECT 
  *,
  NOW() as deletion_timestamp,
  'sugbo_mercado_dashboard_clear' as deletion_reason
...
INSERT INTO cleanup_log (
  ...
  jsonb_build_object(
    ...
    'backup_tables', ARRAY['deleted_sugbo_transactions_backup', 'deleted_sugbo_purchase_orders_backup'],
    'completion_timestamp', NOW(),
    'reason', 'Dashboard sales and orders data clear for Sugbo Mercado'
  ),
  NOW()
);