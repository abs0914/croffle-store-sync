-- Simple Enhanced Product-Recipe Linking

-- Step 1: Link existing recipes to product catalog using fuzzy matching
...
-- Step 2: Create templates for products that have no existing template match
...
-- Step 3: Use deployment function to create recipes from templates
SELECT deploy_and_fix_recipe_templates_to_all_stores();

-- Step 4: Final linking pass for any remaining products
UPDATE product_catalog 
SET recipe_id = r.id,
    updated_at = NOW()
FROM recipes r
