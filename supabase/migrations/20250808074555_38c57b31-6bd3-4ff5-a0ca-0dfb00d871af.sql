...
-- 1.a Update app_users policy to use secure function and drop vulnerable function
DO $$
BEGIN
  -- Update policy to use secure helper
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'app_users' AND policyname = 'admins_owners_can_write'
  ) THEN
    EXECUTE $$ALTER POLICY "admins_owners_can_write" ON public.app_users
             USING (public.is_admin_or_owner())
             WITH CHECK (public.is_admin_or_owner())$$;
  END IF;
  
  -- Drop vulnerable function if present
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'is_admin_or_owner_user'
  ) THEN
    EXECUTE 'DROP FUNCTION public.is_admin_or_owner_user();';
  END IF;
END$$;

-- 1.b Harden SECURITY DEFINER functions by setting explicit search_path
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Users can view cashiers for their stores"
      ON public.cashiers
      FOR SELECT
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Managers and above can create cashiers"
      ON public.cashiers
      FOR INSERT
      WITH CHECK (
        EXISTS (
          ...
          WHERE au.user_id = auth.uid()
            ...
            AND (
              au.role IN ('admin','owner') OR (au.role = 'manager' AND cashiers.store_id = ANY (au.store_ids))
            )
        )
      );$$;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Managers and above can delete cashiers"
      ON public.cashiers
      FOR DELETE
      USING (
        EXISTS (
