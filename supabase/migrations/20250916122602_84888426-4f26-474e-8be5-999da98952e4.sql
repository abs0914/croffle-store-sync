-- Fix backfill function to handle JSONB array properly  
...
BEGIN
  ...
  FOR v_transaction IN
    ...
    LIMIT p_limit
  LOOP
    -- Skip if items is null or not an array
    IF v_transaction.items IS NULL OR jsonb_typeof(v_transaction.items) != 'array' THEN
      RAISE NOTICE 'Skipping transaction % - items is not a valid array', v_transaction.id;
      CONTINUE;
    END IF;
