-- Simplified Product Catalog Standardization Migration
-- Focus on standardizing existing products across stores

-- Step 1: Ensure all stores have the same base products by copying from existing product catalog
...
INSERT INTO product_catalog (
  ...
  recipe_id, is_available, is_active, created_at, updated_at
)
SELECT 
  mp.store_id,
  mp.product_name,
...
SET 
  recipe_id = ur.id,
  cost = COALESCE(ur.total_cost, product_catalog.cost),
  price = CASE 
    WHEN ur.total_cost > 0 AND ur.total_cost > product_catalog.cost 
    THEN GREATEST(ur.total_cost * 2.5, product_catalog.price)
...
-- Step 5: Create store consistency validation
