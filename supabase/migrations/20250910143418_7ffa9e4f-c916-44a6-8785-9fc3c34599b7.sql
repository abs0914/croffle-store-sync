-- COMPREHENSIVE PREMIUM PRODUCT INVENTORY MAPPING FIXES (FIXED v3)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM recipe_templates WHERE name = 'Premium - Choco Overload') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM recipe_templates WHERE name = 'Premium - Matcha') THEN
    INSERT INTO recipe_templates (
      name,
      category_name,
      description,
  ...
  IF NOT EXISTS (SELECT 1 FROM recipe_templates WHERE name = 'Premium - Dark Chocolate') THEN
    INSERT INTO recipe_templates (
      name,
      category_name,  
      description,
...
-- 3. Add template ingredients for Premium - Choco Overload (removed updated_at)
...
-- 4. Add template ingredients for Premium - Matcha (removed updated_at)
INSERT INTO recipe_template_ingredients (
  recipe_template_id,
  ingredient_name,
  quantity,
...
-- 5. Add template ingredients for Premium - Dark Chocolate (removed updated_at)
INSERT INTO recipe_template_ingredients (
  recipe_template_id,
  ingredient_name,
  quantity,
...
WHERE LOWER(TRIM(item)) = 'choco flakes' AND cost != 2.50;

UPDATE inventory_stock 
SET cost = 2.50, updated_at = NOW()
WHERE LOWER(TRIM(item)) = 'matcha crumble' AND cost != 2.50;

UPDATE inventory_stock 
SET cost = 2.50, updated_at = NOW()
WHERE LOWER(TRIM(item)) = 'dark chocolate sauce' AND cost != 2.50;
...
-- 8. Update inventory unit consistency
UPDATE inventory_stock 
SET unit = 'pieces', updated_at = NOW()
WHERE unit = 'piece';

-- 9. Update recipe total costs
UPDATE recipes 
SET 
  total_cost = (
