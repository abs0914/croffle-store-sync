-- Clean Slate Migration (Enum Compatible): Remove Historical Data and Old Processes
...
-- Step 6: Update existing inventory to use direct serving units (working with enum)
UPDATE inventory_stock SET
    unit = CASE 
        WHEN unit IN ('boxes', 'packs') THEN 'pieces'::inventory_unit
        WHEN unit = 'liters' THEN 'ml'::inventory_unit
        WHEN unit = 'kg' THEN 'g'::inventory_unit
        ELSE unit
    END,
