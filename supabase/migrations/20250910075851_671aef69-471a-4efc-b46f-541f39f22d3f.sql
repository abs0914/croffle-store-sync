...
-- Deduct 0.5 Regular Croissant from Sugbo Mercado
...
-- Deduct 0.5 Whipped Cream from Sugbo Mercado
UPDATE inventory_stock
SET 
    stock_quantity = stock_quantity - 0.5,
    updated_at = NOW()
...
-- Create inventory movement record for Mini Croffle missed deduction
INSERT INTO inventory_movements (
    ...
    '5721eaa4-4a8e-4b56-b401-7e7b0e0b6b57',
    ist.id,
    'sale',
    -0.5,
    'transaction',
    'b93c0680-2232-4ffd-a474-54819dc5af12',
    'Retroactive deduction for Mini Croffle transaction 20250910-5721-152838',
    NOW()
...
-- Deduct 1.0 Regular Croissant from Sugbo Mercado
UPDATE inventory_stock
SET 
...
-- Deduct 1.0 Whipped Cream from Sugbo Mercado  
UPDATE inventory_stock
SET 
    stock_quantity = stock_quantity - 1.0,
    updated_at = NOW()
...
-- Create inventory movement record for Croffle Overload missed deduction
INSERT INTO inventory_movements (
    store_id,
    inventory_stock_id,
    movement_type,
    ...
    'Retroactive deduction for Croffle Overload transaction 20250910-5808-152955',
    NOW()
FROM inventory_stock ist
WHERE ist.store_id = '5721eaa4-4a8e-4b56-b401-7e7b0e0b6b57'
    AND ist.item IN ('REGULAR CROISSANT', 'Whipped Cream')
...
-- Check Mini Croffle template quantities are now 0.5
SELECT 
    'Mini Croffle Template Quantities' as check_type,
    ingredient_name,
    quantity
...
-- Check product-recipe linkages are now complete
SELECT 
    'Product Recipe Linkages' as check_type,
    COUNT(*) as total_products,
    COUNT(recipe_id) as linked_products,
    COUNT(*) - COUNT(recipe_id) as orphaned_products
FROM product_catalog 
WHERE LOWER(TRIM(product_name)) IN ('mini croffle', 'croffle overload');

-- Check current inventory levels at Sugbo Mercado
SELECT 
    'Current Inventory Levels' as check_type,
    item,
    stock_quantity
