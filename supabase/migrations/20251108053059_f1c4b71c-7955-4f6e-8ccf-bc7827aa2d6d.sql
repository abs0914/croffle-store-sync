-- Fix type casting issue in the cleanup function
...
BEGIN
  ...
  -- Clean up orphaned inventory movements (fix type casting)
  DELETE FROM inventory_movements im
  WHERE im.reference_type = 'transaction'
    AND NOT EXISTS (
      SELECT 1 FROM transactions t WHERE im.reference_id = t.id::text
    );
