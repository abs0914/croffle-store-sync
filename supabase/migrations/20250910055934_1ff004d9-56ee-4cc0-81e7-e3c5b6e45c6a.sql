-- Drop existing function and recreate with proper UUID handling
DROP FUNCTION IF EXISTS public.insert_inventory_movement_safe(uuid,text,numeric,numeric,numeric,text,text,text,text);

-- Recreate function with improved UUID validation and casting
CREATE OR REPLACE FUNCTION public.insert_inventory_movement_safe(
    ...
    p_created_by text DEFAULT NULL     -- Accept as text first
) RETURNS void
LANGUAGE plpgsql
AS $function$
DECLARE
