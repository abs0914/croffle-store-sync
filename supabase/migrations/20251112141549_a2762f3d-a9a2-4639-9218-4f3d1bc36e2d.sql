-- Fix ambiguous column reference in purchase order number generation
...
BEGIN
  LOOP
    ...
    v_order_number := 'PO-' || TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-' || LPAD((order_count + 1 + attempt)::TEXT, 3, '0');
    
    -- Check if this order number already exists
    IF NOT EXISTS (
      SELECT 1 FROM public.purchase_orders po WHERE po.order_number = v_order_number
    ) THEN
