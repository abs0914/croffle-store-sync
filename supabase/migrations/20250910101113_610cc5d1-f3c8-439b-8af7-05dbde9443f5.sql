-- Clear Sales Data for Sugbo Mercado (IT Park, Cebu) - September 10, 2025 (Fixed)
...
-- Step 3: Create backup of inventory sync audit records
CREATE TABLE IF NOT EXISTS temp_deleted_inventory_sync_backup AS
SELECT 
  isa.*,
  'CLEARED_' || to_char(now(), 'YYYY_MM_DD_HH24_MI_SS') as backup_reason,
  now() as backup_timestamp
FROM inventory_sync_audit isa
JOIN transactions t ON isa.transaction_id = t.id
WHERE t.store_id = 'd7c47e6b-f20a-4543-a6bd-000398f72df5'
  AND DATE(t.created_at AT TIME ZONE 'Asia/Manila') = '2025-09-10'
  AND t.status = 'completed';

-- Step 4: Get current cumulative totals before cleanup
DO $$
DECLARE
...
-- Step 5: Delete inventory sync audit records first (to avoid foreign key constraint)
...
-- Step 6: Delete BIR audit logs for today's transactions
...
-- Step 7: Delete transaction items (due to foreign key constraints)
...
-- Step 9: Update BIR cumulative sales
...
-- Step 10: Verification query to confirm cleanup
DO $$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE 'Backup tables: temp_deleted_transactions_backup, temp_deleted_transaction_items_backup, temp_deleted_inventory_sync_backup';
END $$;