-- Phase 1: Fix miscategorized inventory items
-- Update packaging items that are incorrectly categorized as base_ingredient
UPDATE inventory_stock 
SET item_category = 'packaging'
WHERE item_category = 'base_ingredient' 
AND LOWER(item) IN ('chopstick', 'wax paper', 'napkin', 'takeout bag', 'cup lid', 'straw', 'spoon', 'fork');

-- Create a comprehensive recipe ingredient mapping system
CREATE TABLE IF NOT EXISTS recipe_ingredient_categories (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_category TEXT NOT NULL,
  ingredient_category ingredient_item_category NOT NULL,
...
-- Create function to complete recipes with all required ingredients
...
BEGIN
  ...
  LOOP
    ...
    FOR category_record IN 
      SELECT * FROM recipe_ingredient_categories 
      WHERE product_category = product_category OR product_category = 'default'
    LOOP
        ...
        IF NOT EXISTS (
          ...
          IF EXISTS (
            ...
            INSERT INTO recipe_ingredients (
              ...
              END,
              CASE 
                WHEN category_record.ingredient_category = 'packaging' THEN 'pieces'
                ELSE 'servings'
              END,
              COALESCE((
