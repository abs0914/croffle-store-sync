-- Final Fixed Diagnostic Queries for PostgreSQL with JSON arrays
...
-- 5. Check for cashiers assigned to Robinsons North (fixed JSON syntax)
SELECT user_id, first_name, last_name, store_ids 
FROM app_users 
WHERE role = 'cashier' 
  AND is_active = true 
  AND (store_ids::jsonb ? 'a12a8269-5cbc-4a78-bae0-d6f166e1446d'
...
-- 9. Simple check - what does store_ids column look like?
