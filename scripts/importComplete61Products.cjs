...
/**
 * Import Complete 61 Products
 * 
 * This script imports all 61 products from the complete recipe data provided by the user.
 */
...
// Complete 61 Products Data from user input
...
async function clearExistingData() {
  ...
  const getIngredientsOptions = {
    hostname: 'bwmkqscqkfoezcuzgpwq.supabase.co',
    port: 443,
    path: '/rest/v1/recipe_template_ingredients?select=id',
    method: 'GET',
    headers
  };
  
  const existingIngredients = await makeRequest(getIngredientsOptions);
  console.log(`   Found ${existingIngredients.length} existing ingredients to clear`);
  ...
  const existingTemplates = await makeRequest(getTemplatesOptions);
  console.log(`   Found ${existingTemplates.length} existing templates to clear`);
...
async function main() {
  try {
    console.log('🚀 IMPORTING COMPLETE 61 PRODUCTS');
    console.log('='.repeat(60));

    await authenticateAdmin();

    // Don't clear existing data to avoid foreign key issues
    console.log('📋 Adding missing recipe templates (keeping existing ones)...');
    ...
    const recipes = completeRecipeData.reduce((acc, item) => {
    ...
    const allIngredients = completeRecipeData.map(item => item.ingredient_name);
    ...
    if (successCount === recipeNames.length) {
      console.log('\n🎯 SUCCESS: All recipe templates imported successfully!');
      console.log('\n🔄 NEXT STEPS:');
