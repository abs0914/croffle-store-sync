...
/**
 * Final Recipe Upload Script
 * 
 * Usage:
 *   node scripts/finalUpload.cjs
 * 
 * This script uses valid units and correct workflow.
 */
...
// Unit mapping to convert invalid units to valid ones
const unitMapping = {
  'piece': 'pieces',
  'shot': 'ml',
  'cubes': 'pieces',
...
// Recipe data with corrected units
const recipes = [
  // Classic Croffles
  ...
  // Coffee Drinks (using ml instead of shot/pump)
  ...
  for (const recipe of recipes) {
    try {
      ...
      if (existing.length > 0) {
        ...
        // Create new template as INACTIVE first
        const createOptions = {
          hostname: 'bwmkqscqkfoezcuzgpwq.supabase.co',
          port: 443,
      ...
      for (const ingredient of recipe.ingredients) {
        ...
        if (items.length === 0) {
          // Create commissary item using valid units
      ...
      if (ingredientInserts.length > 0) {
        ...
        await makeRequest(activateOptions, {
          is_active: true,
          updated_at: new Date().toISOString()
        });
      }
