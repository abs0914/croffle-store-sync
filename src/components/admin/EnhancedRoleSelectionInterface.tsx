...
export function EnhancedRoleSelectionInterface({
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <Select value={selectedRole} onValueChange={onRoleChange} disabled={disabled}>
          <SelectTrigger className="w-full">
            <SelectValue placeholder="Select a role">
              {selectedRoleDefinition && <div className="flex items-center gap-2">
                  {React.createElement(ROLE_ICONS[selectedRole as keyof typeof ROLE_ICONS] || Shield, {
                className: "h-4 w-4"
              })}
          ...
          <SelectContent>
            ...
            const roleKey = Object.keys(USER_ROLE_DEFINITIONS).find(key => USER_ROLE_DEFINITIONS[key].name === role.name);
            if (!roleKey) return null;
            const IconComponent = ROLE_ICONS[roleKey as keyof typeof ROLE_ICONS] || Shield;
            return <SelectItem key={roleKey} value={roleKey}>
        ...
        {userRoleDefinition && userRoleDefinition.hierarchy < 100 && <div className="flex items-center gap-2 text-xs text-muted-foreground">
