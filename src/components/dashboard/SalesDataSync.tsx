...
export default function SalesDataSync({
  storeId
}: SalesDataSyncProps) {
    ...
    const checkSyncStatus = async () => {
      ...
      try {
        ...
        const {
          data: metricsData,
          error: metricsError
        } = await supabase.from('store_metrics').select('*').eq('store_id', storeId).eq('metric_date', today).single();

        // Check transactions data
        const {
          data: transactionsData,
          error: transactionsError
        } = await supabase.from('transactions').select('id, total, created_at').eq('store_id', storeId).eq('status', 'completed').gte('created_at', `${today}T00:00:00`).lte('created_at', `${today}T23:59:59`);
        const issues: string[] = [];
