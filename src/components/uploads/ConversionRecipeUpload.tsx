...
export const ConversionRecipeUpload = () => {
  ...
  const downloadMultiIngredientTemplate = () => {
    let csvContent = `Conversion Name,Input Items,Input Quantities,Input UOMs,Output Item,Output Qty,Output UOM,Notes\n`;
    ...
    if (commissaryItems.length >= 2) {
      ...
      csvContent += `Multi-Ingredient Combo,${inputItems},${inputQtys},${inputUOMs},Combined Product,1,combo,Multi-ingredient conversion\n`;
    } else {
      csvContent += `Croissant + Cream Combo,"Croissant Box|Whipped Cream Container","1|7","box|pieces",Regular Croissant + Whipped Cream,1,combo,Multi-ingredient combo\n`;
    }

    csvContent += `Whipped Cream Mix + Monalisa,"Whipped Cream Mix|Monalisa Cream","12|12","bags|liters",Premium Whipped Cream,12,piping bags,Premium cream mixture\n`;
  ...
  const downloadMultiRowTemplate = () => {
    ...
    if (commissaryItems.length >= 2) {
      ...
      items.forEach((item, index) => {
        csvContent += `Multi-Row Combo,${item.name},${index === 0 ? '1' : '7'},${item.uom},${outputName},1,combo,Multi-ingredient via multiple rows\n`;
      });
    } else {
      csvContent += `Croissant + Cream Combo,Croissant Box,1,box,Regular Croissant + Whipped Cream,1,combo,Multi-ingredient combo\n`;
      csvContent += `Croissant + Cream Combo,Whipped Cream Container,7,pieces,Regular Croissant + Whipped Cream,1,combo,Multi-ingredient combo\n`;
    }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <ul className="list-disc list-inside space-y-1">
            ...
            <li><strong>Required columns:</strong> Conversion Name, Output Item, Output Qty, Output UOM</li>
