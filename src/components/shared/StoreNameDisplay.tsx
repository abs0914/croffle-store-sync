  ...
  switch (variant) {
    case "badge":
      return (
        <Badge 
          variant="outline" 
          className={cn("gap-1 font-normal", sizeClasses[size], className)}
        >
    ...
    case "title":
      return (
        <h2 className={cn("font-semibold flex items-center gap-2", sizeClasses[size], className)}>
          {showLogo && currentStore.logo_url && (
            <img
              src={currentStore.logo_url}
              alt={currentStore.name}
              className={cn("object-cover", size === "sm" ? "h-5 w-5" : size === "md" ? "h-7 w-7" : "h-9 w-9")}
            />
          )}
          {currentStore.name}
        </h2>
