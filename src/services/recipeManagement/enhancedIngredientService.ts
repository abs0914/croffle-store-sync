...
export const saveEnhancedRecipeIngredients = async (
  ...
  try {
    ...
    // Insert new ingredients with enhanced data
    ...
    if (bulkMappings.length > 0) {
      // Find corresponding inventory stock items for bulk mappings
      for (const mapping of bulkMappings) {
        const { data: inventoryItem } = await supabase
          .from('inventory_stock')
          .select('id')
          .eq('item', mapping.bulk_item_name)
          .maybeSingle();

        if (inventoryItem) {
          // Create or update conversion mapping
          await supabase
            ...
            .upsert({
              ...
              onConflict: 'inventory_stock_id,recipe_ingredient_name,recipe_ingredient_unit'
            });
        }
      }
    }
...
export const saveEnhancedTemplateIngredients = async (
  ...
  try {
    ...
    // Insert new template ingredients with enhanced data
    const ingredientData = ingredients.map(ingredient => ({
...
export const calculateEnhancedRecipeCost = (ingredients: EnhancedRecipeIngredient[]): number => {
  return ingredients.reduce((total, ingredient) => {
    const cost = ingredient.cost_per_recipe_unit || 0;
