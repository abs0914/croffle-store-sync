-- OPTION A: Standardize name and materialize product_ingredients for "Cookies Cream Croffle"
...
WHERE product_name = 'Cookies  Cream Croffle'
...
WHERE pc.product_name IN ('Cookies  Cream Croffle', 'Cookies Cream Croffle')
ORDER BY pc.store_id;

-- 4) Count existing product_ingredients (should be 0 per earlier checks)
...
SET product_name = 'Cookies Cream Croffle', updated_at = NOW()
WHERE product_name = 'Cookies  Cream Croffle';
...
WITH targets AS (
  ...
  FROM product_catalog pc
  JOIN recipes r ON r.id = pc.recipe_id
  WHERE pc.product_name = 'Cookies Cream Croffle'
), candidates AS (
  SELECT t.product_catalog_id,
         ist.id AS inventory_stock_id,
...
FROM product_catalog
WHERE product_name IN ('Cookies Cream Croffle', 'Cookies  Cream Croffle')
