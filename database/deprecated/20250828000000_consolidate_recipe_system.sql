...
-- This migration optimizes the database schema for the unified recipe management system

-- Step 1: Ensure all required columns exist with proper constraints
-- DO $$
-- BEGIN
...
-- Step 3: Create index for better performance on category lookups
...
-- Step 5: Create function to map template categories to POS categories
...
-- Step 6: Create function to get or create category for a store
...
-- Step 7: Create trigger to automatically assign categories when product_catalog entries are created
...
-- Step 8: Update existing product_catalog entries that don't have categories
-- UPDATE product_catalog 
-- SET category_id = get_or_create_category(store_id, rt.category_name)
-- FROM recipes r
-- JOIN recipe_templates rt ON r.template_id = rt.id
...
-- Step 9: Create view for easy recipe management reporting
...
-- Step 10: Create function to safely clear recipe data (deactivate instead of delete)
